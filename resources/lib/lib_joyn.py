...
class lib_joyn(Singleton):
	...
	def get_auth_token(self,
		...
		if username is not None and password is not None:
			...
			# GENERATE COOKIE
			cookie_filename = f'{}.cookie.tmp', sha512(str(time()).encode('utf-8')).hexdigest()
			...
			try:
				...
				# CHECK LANG
				a = request_helper.get_json_response(url=f'https://auth.7pass.de/registration-setup-srv/public/list?acceptlanguage=undefined&requestId={}',request_id,
									config=self.config,
									cookie_file=cookie_file,
									no_cache=True)

				# CHECK MAIL ADDRESS
				a = request_helper.post_json(url=f'https://auth.7pass.de/users-srv/user/checkexists/{}', request_id,
								config=self.config,
				...
				a = request_helper.get_json_response(url=f'https://auth.7pass.de/token-srv/prelogin/metadata/{}?acceptLanguage=de-de', id_dict['track_id'][0] if login_flow_exp == False else id_dict['cd1'][0],
									config=self.config,
									cookie_file=cookie_file,
									no_cache=True)
				...
				if login_flow_exp == False:
					# CONTINUE
					a, b = request_helper.get_url(url=f'https://auth.7pass.de/login-srv/precheck/continue/{}', id_dict['track_id'][0] if login_flow_exp == False else id_dict['cd1'][0],
									config=self.config,
									method='POST',
									cookie_file=cookie_file,
									return_final_url=True,
